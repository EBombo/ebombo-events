...
import React, { forwardRef, useMemo, useState } from "reactn";
...
export const SharpButton = forwardRef(({ className, ...props }, ref) => {
  ...
  const prefixIcon = useMemo(() => {
    ...
    if (props.disabled)
      return <Image height="24px" width="24px" src={`${config.storageUrl}/resources/emoji-dead-face.svg`} />;
    ...
    if (props.prefixIcon === "satisfied" && onHover)
      iconUrl = `${config.storageUrl}/resources/emoji-satisfied-happy-face.svg`;
  ...
  return (
    <StyledSharpButton
      className={`${className} relative whitespace-nowrap inline-block
      px-6 py-4
      text-white text-lg border-secondary border-2
      ${
        !props.disabled &&
        "hover:shadow-none hover:top-[3px] hover:left-[-5px] active:top-[2px] active:left-[-2px] active:shadow-sharp-sm"
      } focus:shadow-sharp-sm shadow-sharp`}
      ref={ref}
