...
export const SeoContainer = (props) => {
  ...
  useEffect(() => {
    const fetchSeo = async () => {
      const seoQuery = await firestore.doc(`seo/${hostId}`).get();
  ...
  const saveSEO = async (data) => {
    try {
      ...
      await firestore.doc(`seo/${hostId}`).set(SEO);
  ...
  return (
    <SeoContainerCss>
      ...
      <form onSubmit={handleSubmit(saveSEO)} autoComplete="off" noValidate>
        {seo.map((currentSeo, index) => (
          <fieldset key={index}>
            <Input
              type="text"
