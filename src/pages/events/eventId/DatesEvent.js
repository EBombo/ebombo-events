...
export const DatesEvent = (props) => {
  const [currentDate, setCurrentDate] = useState({ id: firestore.collection("event").doc().id });
    ...
    <div>
      ...
      <div className="grid grid-cols-4 gap-3 w-9/12 rounded-md border-2 border-grayLighten py-6 px-4 mb-9">
        <div>
          ...
          <DatePicker format="ll" onChange={(event) => setCurrentDate({ month: event })} />
        </div>

        <div>
          <div className="text-secondary mb-4">Inicio del evento</div>
          <TimePicker className="w-full" onChange={(event) => setCurrentDate({ start: event })} />
        </div>

        <div>
          <div className="text-secondary mb-4">Fin del evento</div>
          <TimePicker className="w-full" onChange={(event) => setCurrentDate({ end: event })} />
        </div>
        ...
        <div className="flex">
          <ButtonAnt
            ...
            onClick={() => props.setDates([currentDate])}
          >
            Adicionar fecha
          </ButtonAnt>
        </div>
      ...
      {props.dates?.length ? (
        <>
          ...
          {props.dates.map((date) => {
            return (
              <div
                className="grid grid-cols-5 gap-3 w-9/12 rounded-md border-2 border-grayLighten py-6 px-4"
                key={date.id}
              >
                <div>
