...
export const UpdateSubscriptionModal = ({
  ...
  const fetchPreviewSubscriptionUpdate = async () => {
    try {
      ...
      const customerQuery = await firestore.doc(`customers/${authUser.id}`).get();

      const customer = customerQuery.data();

      const { response, error } = await Fetch(
        `${config.serverUrl}/api/subscriptions/${subscription.id}/review`,
        "POST",
        {
  ...
  if (!subscription || !updateSubscriptionData)
    return (
      <ModalContainer footer={null} closable={false} padding={"0rem 0rem 2rem 0rem"}>
  ...
  return (
    ...
    >
      ...
      <div
        className={`text-blackDarken px-8 pb-4
        overflow-hidden transition-height duration-300 ease-in-out ${
          previewSubscriptionUpdateResponse || previewSubscriptionUpdateError ? "max-h-[600px]" : "max-h-[200px]"
        }`}
      >
          ...
          <>
            <div className="font-bold text-xl mb-8">
              ...
              <div className="float-right text-base">
                {getCurrencySymbol[updateSubscriptionData.price.currency]}{" "}
                {amountToString(updateSubscriptionData.price.amount)}
              </div>
            </div>
          </>
        )}
